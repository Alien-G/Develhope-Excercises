...
public class Main {
     ...
     * A1: In this case a do-while loop would be best. We want to prompt the user to input their action at least once,
     * and we do not know how many actions the user will take.
     *
     * Q2: We have a list of 1000 users to send emails to
     * A2: We should use a for-each loop because we want to iterate over each user in the list and send an email to each one.
     *
     * Q3: We want to print out the first 80 customer names from our bank database
     * A3: Since we know the exact number of names we want to print (80), we should use a for i loop in order to print them out.
     *
     * Q4: We want to read a file 100 lines at a time, without loading the full file into our program
     * A4: We should use a while loop. As we only want to read the file if there are lines until we have read 100 lines,
     * the loop should continue until we have read the entire file.
     */
}
